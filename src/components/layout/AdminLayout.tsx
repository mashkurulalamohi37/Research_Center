...
import { 
  ...
  Image, BarChart3, FileText, Settings, LogOut, Sun, Moon, 
...
export const AdminLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-navy-950 flex text-slate-900 dark:text-slate-100 transition-colors duration-200">
      ...
      {/* Admin Sidebar */}
      <aside
        className={`fixed lg:static top-0 bottom-0 left-0 z-50 w-64 bg-white dark:bg-slate-950/98 border-r border-slate-200 dark:border-slate-800 flex flex-col justify-between p-4 transition-transform duration-300 shadow-xl lg:shadow-none ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}
      >
        <div className="overflow-y-auto pr-1">
          {/* Brand Header */}
          <div className="flex items-center justify-between px-3 py-3 mb-4 border-b border-slate-200 dark:border-slate-800">
            <Link to="/" className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl bg-purple-500/15 border border-purple-500/30 flex items-center justify-center text-purple-600 dark:text-purple-400 shadow-sm">
                <ShieldAlert className="w-5 h-5" />
              </div>
              <div className="flex flex-col">
                <span className="text-base font-black tracking-wider text-slate-900 dark:text-slate-100 font-sans">AIRC ADMIN</span>
                <span className="text-[11px] text-purple-600 dark:text-purple-400 font-mono font-bold">DIRECTORATE CMS</span>
              </div>
            </Link>
        ...
        <div className="pt-4 border-t border-slate-200 dark:border-slate-800 space-y-3">
          <div className="flex items-center gap-3 px-2">
            <img
              ...
              alt="Prof. Sarah Lin"
              className="w-9 h-9 rounded-xl object-cover border border-purple-500/40 shadow-sm"
            />
            <div className="flex-1 min-w-0">
              <div className="text-xs font-bold text-slate-900 dark:text-slate-100 truncate">Prof. Dr. Sarah Lin</div>
              <div className="text-[10px] text-purple-600 dark:text-purple-400 font-mono font-semibold truncate">Founding Director</div>
            </div>
          </div>
